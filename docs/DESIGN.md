...
**Architecture**: Pluggable detector model behind an abstract base class (`DetectorBase`). The implementation uses a production-ready `YOLODetector` (YOLOv8 + MobileNetV3 + OSNet).
...
**My Decision**: I agreed with the abstraction principle but simplified to a direct abstract base class + concrete implementations. The factory pattern added unnecessary complexity. The current approach — `DetectorBase` → `YOLODetector` — is simpler to understand and equally extensible.
...
| Model | YOLOv8 (pluggable via DetectorBase) | Best speed/accuracy for retail person detection |
